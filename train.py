...
def main(args):
    ...
    for i_iter in trange(1, args.iters + 1):
        ...
        output_path = os.path.join('/content/EvINR_NeRV_plus_plus/logs', 'output_image.png')
        ...
        image_data = (intensity1*255).astype(np.uint8)
        image = Image.fromarray(image_data)
        output_path = os.path.join('/content/EvINR_NeRV_plus_plus/logs', 'output_image_0.001.png')
        image.save(output_path)
    ...
    with torch.no_grad():
        file_path = "/content/EvINR_NeRV_plus_plus/ECD/slider_depth/images.txt"  # 替换为你的 TXT 文件路径
        ...
        with open(file_path, "r") as file:
          for line in file:
            ...
            image_path = os.path.join('/content/EvINR_NeRV_plus_plus/ECD/slider_depth',image_path)
